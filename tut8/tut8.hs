module Tut8 where
  ...
  solveRPN = head . foldl foldingFunction [] . words
    where   foldingFunction (x:y:ys) "*" = (x >>= (\a -> y >>= (\b -> return (a * b)))):ys
            foldingFunction (x:y:ys) "+" = (x >>= (\a -> y >>= (\b -> return (a + b)))):ys
            foldingFunction (x:y:ys) "-" = (y >>= (\a -> x >>= (\b -> return (a - b)))):ys
            foldingFunction (x:y:ys) "^" = (y >>= (\a -> x >>= (\b -> return (a ** b)))):ys
            foldingFunction (x:y:ys) "/" = division x y ys
            foldingFunction (x:xs) "ln" = logarithm x xs
            foldingFunction xs "sum" = [foldM (fmap . (+)) 0 xs]
  ...
  division x y ys = (y >>= (\a -> x >>= (\b -> return (a / b)))):ys
